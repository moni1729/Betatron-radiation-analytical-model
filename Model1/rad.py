...
def Sb(x):
    return (9 * np.sqrt(3) / (16 * np.pi)) * (Lambda ** 4) * (x ** 3) * \
        scipy.integrate.quad(lambda v: scipy.integrate.quad(
            ...
            v, np.inf)[0], 0, np.inf)[0]+np.random.poisson(1000)
Sb = np.vectorize(Sb)
...
fig.savefig('results/double_differential', dpi=300)
plt.close(fig)
...
cbar = fig.colorbar(hm, ax=ax)
cbar.set_label('$\\frac{dI}{d\\phi_x}$ (eV)')
fig.savefig('results/double_differential_matched', dpi=300)
plt.close(fig)
'''
...
fig.savefig('results/dist.png', dpi=300)
plt.close(fig)
'''
...
cbar = fig.colorbar(cm.ScalarMappable(norm=colors.Normalize(vmin=vmin, vmax=vmax)), ax=ax)
cbar.set_label('$\\frac{dI}{d\\Omega}$ (eV)')
fig.savefig('results/dist_matched.png', dpi=300)
plt.close(fig)
'''
...
fig.savefig('results/1d_dist.png', dpi=300)
plt.close(fig)
...
ax.set_yscale('log')
fig.savefig(f'results/spectrum.png', dpi=400)
plt.close(fig)
